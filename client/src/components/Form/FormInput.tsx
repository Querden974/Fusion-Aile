...
import {HandleInputs} from "@/views/Reservation/functions/Handle";
    ...
    return (
        <div className="flex flex-col gap-1">
            ...
            <ShadInput type={type} name={name} className={`input bg-base-100 ${state.error ? "input-error" : ""}`} value={state.value}
                   onChange={(e:ChangeEvent<HTMLInputElement>):void => HandleInputs(e,state,setState)} placeholder="Quel est votre nom?" required/>
        </div>
    )
}
    ...
    return (
        <div className="flex flex-col gap-1">
            ...
            <select value={state} defaultValue="none"
                ...
                {options.map((item:{code:string,nom:string}, index:number) => (
